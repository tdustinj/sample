...
class UpdateShippingInformationForNeweggOrdersCommand extends Command
{
     ...
     * @var string
     */
    protected $name = 'command:update-shipping-information-for-newegg-orders';
     ...
     * @var string
     */
    protected $description = 'Send Tracking Information To Newegg for Orders';
    ...
    public function fire()
    {
        ...
        $fulfilledItems = ShippingTransaction::where('platform_updated_status', '=', 'NOT_UPDATED')->where('fulfillment_shipment_status', '=', 'SHIPPED')->where('partner', '=', 'newegg')->where('status', '=', 1)->get();
        ...
        if(sizeof($fulfilledItems)) {
            ...
            $header_array =array('Content-Type:application/xml',
               ...
               'SecretKey: 25c7b685-ad53-4078-b837-a72e70090c1f');  
            $groupedByInvoice = array();
            foreach ($fulfilledItems as $item) {    // we have to combine items based on invoice
                $order = OrdersNewegg::where('OrderNumber', '=', $item['partner_order_number'])->where('SellerPartNumber', '=', $item['description'])->get();
                if(sizeof($order)) {
            ...
            foreach ($groupedByInvoice as $shipment) {
                ...
                if($shipment[0]['NeweggItemNumber']) {  // means we successfully did a lookup in the above code so should be good
                    ...
                    $request = "https://api.newegg.com/marketplace/ordermgmt/orderstatus/orders/". $shipment[0]['partner_order_number'] . "?sellerid=A3YF";
                    ...
                    if ($xml->IsSuccess == 'true') {
                        echo "success\n\r";
                        $msg = "Updated Newegg with shipping info";
                        DB::table('shipping_transactions')->where('invoice_num', $shipment[0]->invoice_num)->where('bol_tracking', $shipment[0]['bol_tracking'])->update(array('platform_updated_status' => 'UPDATED', 'updated_at' => $right_now, 'user' => 'Newegg Batch Run API'));                        
                        ...
                        DB::table('shipping_transactions')->where('invoice_num', $shipment[0]->invoice_num)->where('bol_tracking', $shipment[0]['bol_tracking'])->update(array('platform_updated_status' => 'SHIPPING CONFIRMATION ERROR', 'updated_at' => $right_now, 'user' => 'Newegg Batch Run API'));                        
                    }   
                    //die;                
